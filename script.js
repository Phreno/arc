...
function televerseFichier(stringData = global.svg.innerHTML, prefix = "export") {
  const svgData = `<?xml version="1.0" standalone="no"?><svg width="5cm" height="4cm" version="1.1" xmlns="http://www.w3.org/2000/svg">${stringData}</svg>`
  const link = document.createElement("a");
  link.setAttribute("download", `${prefix}.svg`);
  link.setAttribute(
    "href",
    `data:image/svg+xml; charset=UTF-8,%EF%BB%BF${encodeURI(svgData)}`
  );
